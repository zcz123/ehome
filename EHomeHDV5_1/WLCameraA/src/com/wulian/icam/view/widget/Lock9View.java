...
public class Lock9View extends ViewGroup {
	...
	private void initFromAttributes(AttributeSet attrs, int defStyleAttr) {
		...
		lineColor = a
				.getColor(R.styleable.Lock9View_lock9_lineColor, lineColor);
		float lineWidth = 20.0f;
		lineWidth = a.getDimension(R.styleable.Lock9View_lock9_lineWidth,
				lineWidth);
		...
		bitmap = Bitmap.createBitmap(dm.widthPixels, dm.widthPixels,
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (nodeAt == null && currentNode == null) { // 不需要画线，之前没接触点，当前也没接触点
				...
				if (currentNode == null) { // 第一个点 nodeAt不为null
					...
					canvas.drawLine(currentNode.getCenterX(),
							currentNode.getCenterY(), event.getX(),
							event.getY(), paint);
				} else { // 移动到新点
					canvas.drawLine(currentNode.getCenterX(),
							currentNode.getCenterY(), nodeAt.getCenterX(),
							nodeAt.getCenterY(), paint);// 画线
