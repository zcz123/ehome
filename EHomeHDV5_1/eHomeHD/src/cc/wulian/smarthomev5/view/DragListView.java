...
public class DragListView extends ListView{
	...
	private int startPosition;  //手指点击准备拖动的时候,当前拖动项在列表中的位置.
...
//	private ItemInfo mDragItemInfo;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN && !isLock && !isMoving && !isDragItemMoving) {
			...
			AutoActionTaskAdapter adapter = (AutoActionTaskAdapter) getAdapter();
			...
			// 获取可拖拽的图标
			View draggerIcon = dragger.findViewById(R.id.device_short_cut_menu_sort_ll);
...
//				dragger.removeAllViews();
	...
	private void hideDropItem(){
		final AutoActionTaskAdapter adapter = (AutoActionTaskAdapter)this.getAdapter();
...
//				OnMove(moveY);
	...
	private void testAnimation(int y){
		final AutoActionTaskAdapter adapter = (AutoActionTaskAdapter) getAdapter();
