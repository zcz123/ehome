...
public class AddDeviceToWifiFragmentDialog extends DialogFragment {
	private static final String TAG = AddDeviceToWifiFragmentDialog.class
			.getSimpleName();
	...
	public static void showDeviceDialog(FragmentManager fm,
		...
		AddDeviceToWifiFragmentDialog fragment = new AddDeviceToWifiFragmentDialog();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mAddDeviceInfoAdapter = new AddDeviceInfoAdapter(this.getActivity(),
				getRemainDevices());
	}
	...
	private class SelectDevicelistener implements MessageListener {
		...
		public void onClickPositive(View contentViewLayout) {
			...
			List<WifiEntity> selectedWifiEntities = new ArrayList<WifiEntity>();
			...
			for (int i = 0; i < lists.size(); i++) {
				if (mAddDeviceInfoAdapter.getBitSet().get(i)) {
					...
					entity.setOperateType(WifiEntity.TYPE_DEVICE);
					...
					selectedWifiEntities.add(entity);
				}
			}
			EventBus.getDefault().post(
					new WifiEvent(CmdUtil.MODE_ADD, selectedWifiEntities, null,
							false));
		}
	...
	public View createView() {
		View view = LayoutInflater.from(getActivity()).inflate(
				R.layout.list_add_device_info, null);
		ListView listView = (ListView) view
				.findViewById(R.id.action_pop_menu_list);
	...
	private List<DeviceInfo> getRemainDevices() {
		if (WifiDataManager.wifiEntities == null) {
			return new ArrayList<DeviceInfo>();
		}
		DeviceInfo deviceInfo = new DeviceInfo();
		deviceInfo.setGwID(AccountManager.getAccountManger().getmCurrentInfo()
				.getGwID());
		List<DeviceInfo> result = deviceDao.findListWifiRemain(deviceInfo,
				WifiDataManager.wifiEntities);
		for (int i = result.size() - 1; i >= 0; i--) {
			DeviceInfo info = result.get(i);
			WulianDevice device = deviceCache.getDeviceByIDEp(getActivity(),
					info.getGwID(), info.getDevID(), info.getDevEPInfo()
							.getEp());
			if (device == null
					|| !device.isDeviceUseable() || !device.isAutoControl(true)) {
				result.remove(info);
			}
		}
		return result;
