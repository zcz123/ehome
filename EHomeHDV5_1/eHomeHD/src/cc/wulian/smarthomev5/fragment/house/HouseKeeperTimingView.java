...
public class HouseKeeperTimingView extends LinearLayout{
	...
	private HouseKeeperEntity mTimeEntity ;
	...
	public HouseKeeperTimingView(Context context) {
		super(context);
	...
	private final RepeatWeekDayView.OnRepeatWeekChangedListener mOnRepeatWeekChangedListener = new RepeatWeekDayView.OnRepeatWeekChangedListener()
	{
		...
		public void onWeekDayChanged( RepeatWeekDayView weekView, String weekDay ) {
			String oldWeek = getTimingTrigger().weekDay;
...
//			updateTime(c.get(Calendar.HOUR_OF_DAY)  ,c.get(Calendar.MINUTE),mSecond);
	...
	private void updateTime(int hour,int minute, int mSecond) {
		this.mTimeEntity.time =getTimeFromHourAndMinute(hour, minute, mSecond);
	}
	...
	public HouseKeeperEntity getTimingTrigger() {
		if(StringUtil.isNullOrEmpty(mTimeEntity.time)){
			mTimeEntity.time = getTimeFromHourAndMinute(Calendar.getInstance().get(Calendar.HOUR_OF_DAY), Calendar.getInstance().get(Calendar.MINUTE), mSecond);
		}
		return mTimeEntity;
	}
	
	public void setmTimingTrigger(HouseKeeperEntity entity ){
		mTimeEntity = entity;
		initTime(entity.time);
		initWeek(entity.weekDay);
	}

	private void updateWeek(String newValue){
		this.mTimeEntity.weekDay = newValue;
	}
