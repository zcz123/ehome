 ...
 */
public class WulianProgressFragment extends WulianFragment
{
	...
	/**
	 * Provide default implementation to return a simple view. Subclasses can override to replace with their own layout. If doing so, the returned view hierarchy
	 * <em>must</em> have a progress container whose id is {@link R.id#progressContainer R.id.progressContainer}, content container whose id is
	 * {@link R.id#listContainer R.id.listContainer} and can optionally have a sibling view id {@link android.R.id#empty android.R.id.empty} that is to be shown
	...
	public void setContentView( View view ){
		ensureContent();
		if (view == null){ throw new IllegalArgumentException("Content view can't be null"); }
	...
	/**
	 * The default content for a ProgressFragment has a TextView that can be shown when the content is empty {@link #setContentEmpty(boolean)}. If you would like
	...
	public void setContentEmpty( boolean isEmpty ){
		ensureContent();
		if (mContentView == null){ throw new IllegalStateException(
	...
	private void ensureContent(){
		if (mContentContainer != null && mProgressContainer != null){ return; }
		View root = getView();
		if (root == null){ throw new IllegalStateException("Content view not yet created"); }
		mProgressContainer = root.findViewById(R.id.progressContainer);
		if (mProgressContainer == null){ throw new RuntimeException(
				"Your content must have a ViewGroup whose id attribute is 'R.id.progressContainer'"); }
		mContentContainer = root.findViewById(R.id.listContainer);
		if (mContentContainer == null){ throw new RuntimeException(
				"Your content must have a ViewGroup whose id attribute is 'R.id.listContainer'"); }
