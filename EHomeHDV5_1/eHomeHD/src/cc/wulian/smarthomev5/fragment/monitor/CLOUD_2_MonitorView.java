...
//	private MonitorSearchCallBackView mMonitorSearchCallBack;
	...
	public CLOUD_2_MonitorView(BaseActivity context, CameraInfo info) {
		super(context, info);
	}
...
//		});
		if (cameraInfo.getCamType() == -1 || cameraInfo.getCamId() == -1) {
			monitorAreaNameSpinner.setSelection(0);
			monitoruidEditText.setText(cameraInfo.getUid());//add mabo 2016/07/13
		} else {
...
//				monitoruidEditText, cameraInfo);
		...
		* @Description: TODO(物联摄像机添加账号与修改账号) 
		* @author ylz
		* @date 2015-4-20 上午11:28:34 
		*
		 */
	public class OnClickListenerImp implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (whetherAllEditTextFilled()) {
				//修改 mabo 2016/7/13
				if (cameraDao.isExistCamera(cameraInfo)) {
	...
	private void getValueFromView() {
		...
		String mUsername = monitorCameraNameEditText.getText().toString().trim();
		...
		cameraInfo.setCamType(CameraInfo.CAMERA_TYPE_CLOUD_2);
		cameraInfo.setUid(mUid);
		cameraInfo.setHost("");
		cameraInfo.setPort(0);
		//修改 add mabo 2016/7/13
		if (mUsername==null) {
	...
	public void jumptoOtherCameraView(CameraInfo info) {
		...
		if (!updateManager.isIcamAppInstalled()) {
			...
			intent.putExtra(CameraInfo.CAMERA_KEY_CAMERATYPE, CameraInfo.CAMERA_TYPE_CLOUD_2+"");
