 ...
 * 冰箱
 */
@DeviceClassify(devTypes = {"Oe"}, category = Category.C_CONTROL)
public class WL_Oe_Fridge extends ControlableDeviceImpl {
    public WL_Oe_Fridge(Context context, String type) {
        super(context, type);
    }
    private String pluginName="Device_Oe_Fridge.zip";
    ...
    //主页
    private static String controlPagePath="file:///android_asset/Device_Oe_Fridge/index.html";
    //管家
    private static String housekeeperPagePath="file:///android_asset/Device_Oe_Fridge/indexForButler.html";
    //设备信息
    private String deviceInfoPagePath="file:///android_asset/Device_Oe_Fridge/LampblackDeviceInfo.html";
    ...
    public void onViewCreated(View view, Bundle saveState) {
        ...
        if(isUsePlugin){
            getPlugin("index.html","Device_Oe_Fridge",mContext);
        }else{
    ...
    public Drawable getDefaultStateSmallIcon() {
        Drawable icon = getResources().getDrawable(R.drawable.device_oe_fridge);
        return icon;
    }*/
    ...
    public String getDefaultDeviceName() {
        ...
        if (StringUtil.isNullOrEmpty(defaultName)) {
            defaultName =getString(cc.wulian.app.model.device.R.string.device_Oe_Fridge);
        }
        return defaultName;
    }
    ...
    public DeviceShortCutSelectDataItem onCreateShortCutSelectDataView(DeviceShortCutSelectDataItem item, LayoutInflater inflater, AutoActionInfo autoActionInfo) {
        if(item == null){
            item = new WL_Oe_Fridge.ShortCutControlableDeviceSelectDataItem(inflater.getContext());
        }
    ...
    public DialogOrActivityHolder onCreateHouseKeeperSelectControlDeviceDataView(LayoutInflater inflater, AutoActionInfo autoActionInfo) {
        if(isDeviceOnLine()){
            ...
            if(HouseKeeperSelectControlDeviceDataFragment.isShowHouseKeeperSelectControlDeviceDataView){
                ...
                if(isUsePlugin){
                    getPlugin("indexForButler.html","Device_Oe_Fridge", DeviceSettingActivity.instance);
                }else{
            ...
            Toast.makeText(getContext(),"设备已离线",Toast.LENGTH_SHORT).show();
            return null;
        }
    }
    /*增加更多菜单项*/
...
//                String setttingFilePath = Preference.getPreferences().getAuCurtainSettingUri();
                SmarthomeFeatureImpl.setData(SmarthomeFeatureImpl.Constants.DEVICEID, WL_Oe_Fridge.this.devID);
                SmarthomeFeatureImpl.setData(SmarthomeFeatureImpl.Constants.GATEWAYID, WL_Oe_Fridge.this.gwID);
                SmarthomeFeatureImpl.setData(SmarthomeFeatureImpl.Constants.EP, WL_Oe_Fridge.this.ep);
                SmarthomeFeatureImpl.setData(SmarthomeFeatureImpl.Constants.EPDATA,WL_Oe_Fridge.this.epData);
//                String setttingFilePath =Preference.getPreferences().getAuCurtainSettingUri();
//                String setttingFilePath ="file:///android_asset/Curtain_Au/setting.html";
                if(isUsePlugin){
                    deviceInfoPagePath=Preference.getPreferences().getHxOeDeivceInfo();
                }
                ...
                pm.getHtmlPlugin(context, pluginName,
                        new PluginsManager.PluginsManagerCallback() {
                            ...
                            public void onGetPluginSuccess(PluginModel model) {
                                ...
                                Preference.getPreferences().saveHxOeDeviceInfo("file:///"+model.getFolder()+"/LampblackDeviceInfo.html");
                            ...
                            public void onGetPluginFailed(final String hint) {
                                if((!Preference.getPreferences().getHxOeDeivceInfo().equals(""))){
                                    return;
                                }
