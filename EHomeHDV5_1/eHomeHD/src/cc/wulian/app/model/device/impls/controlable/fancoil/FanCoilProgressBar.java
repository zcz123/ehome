...
public class FanCoilProgressBar extends View {
	...
	private int height;
	//拖动球  画笔
	private Paint mHeatThumbPaint;
	private Paint mCoolThumbPaint;
	...
	public FanCoilProgressBar(Context context) {
		super(context);
		init(context);

	}

	public FanCoilProgressBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public FanCoilProgressBar(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(Context context) {
		//制热模式拖动球
		mHeatThumbPaint = new Paint();
		mHeatThumbPaint.setAntiAlias(true);
		mHeatThumbPaint.setColor(Color.parseColor("#ec7016"));
		mHeatThumbPaint.setStrokeWidth(1);
		//制冷模式拖动球
		mCoolThumbPaint = new Paint();
		mCoolThumbPaint.setAntiAlias(true);
		mCoolThumbPaint.setColor(Color.parseColor("#7fa82f"));
		mCoolThumbPaint.setStrokeWidth(1);
		// 背景Paint
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		 ...
		 * 为了与背景图片重合,按比例将圆弧半径缩小,其比例为10/13,刚好与背景图片中间的圆弧重合
		 */
		if (w > h) {
			mDiameter = h;
			mRadius = ((mDiameter / 2 - (getPaddingTop() + getPaddingBottom())) * 10) / 13;
		} else {
			mDiameter = w;
			mRadius = ((mDiameter / 2 - (getPaddingLeft() + getPaddingRight())) * 10) / 13;
		}
	...
	private void drawThumb(Canvas canvas){
		...
		if (arcPoint != null) {
			if(isHeat){
				canvas.drawCircle(arcPoint.x, arcPoint.y, circleWidth, mHeatThumbPaint);
			}else{
				canvas.drawCircle(arcPoint.x, arcPoint.y, circleWidth, mCoolThumbPaint);
			}
		}
	}
