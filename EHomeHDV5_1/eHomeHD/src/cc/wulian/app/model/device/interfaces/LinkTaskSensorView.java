...
public class LinkTaskSensorView extends AbstractLinkTaskView{
	...
	private final int adddvalue = 250;
	...
	public LinkTaskSensorView(BaseActivity context, TaskInfo info) {
		super(context, info);
	}
	...
	private View createLinkView() {
		...
		valueUnitText.setText("PPM");
		mValueLink = String.valueOf(250);
		valueSeekBar.setProgress(0);
		valueSeekBar.setMax(2250);
		changeTextView(mValueLink);
		valueEditText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				valueSeekBar.setProgress(progress - adddvalue);
				valueEditText.setSelection(valueEditText.getText().toString().length());
			}
		});
		...
		valueSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
			...
			public void onStopTrackingTouch(SeekBar seekBar) {
				...
				mValueLink = String.valueOf(mSeekProgress + adddvalue);
				changeTextView(mValueLink);
			...
			public void onProgressChanged(SeekBar seekBar, int progress,
				...
				mValueLink = String.valueOf(progress + adddvalue);
				changeTextView(mValueLink);
				valueEditText.setText(mValueLink);
			}
		});
	...
	private void changeTextView(String mValueLink2){
		int values = StringUtil.toInteger(mValueLink2);
		if(values >= 250 && values <= 350){
			valuedegreeText.setText(context.getString(R.string.device_link_task_detection_degree_a));
		}else if(values > 350 && values <= 750){
			valuedegreeText.setText(context.getString(R.string.device_link_task_sensor_degree_in));
		}else if(values > 750 && values <= 1000){
			valuedegreeText.setText(context.getString(R.string.device_link_task_detection_degree_b));
		}else if(values > 1000 && values <= 2500){
			valuedegreeText.setText(context.getString(R.string.device_link_task_detection_degree_d));
		}else{
