...
public class CooArcProgressBar extends View {

	protected static final int MAX_SECTION = 60;
	protected static final int MIN_SECTION = 5;
	// 背景画笔
	...
	// 提供颜色渐变的单位值
	// private float angleUnit = sweepAngle / 256.0f;
	// 画弧形的矩阵区域
	...
	// private OnDownViewValueChanged mDown;
	...
	//
	// public void setOnDownViewValueChanged(OnDownViewValueChanged down) {
	// mDown = down;
	// }
	...
	public CooArcProgressBar(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public CooArcProgressBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public CooArcProgressBar(Context context) {
		super(context);
		init();
	}
	...
	// }
	//
	// public void setSlided(boolean isSlided) {
	 ...
	 */
	/*
	 * private void drawFg(Canvas canvas) { for (i = 0; i < j; i++) {
	 * fgPaint.setColor(Color.rgb(i, 256 - i, 0));
	 * canvas.drawArc(mRadialScoreRect, startAngle + angleUnit * i, angleUnit,
	 * false, fgPaint); } invalidate();
	 * 
	 * }
	 */
...
//		}
		...
		case MotionEvent.ACTION_MOVE:
			setCurrentValue((int) Math
					.round((degree * (MAX_SECTION - MIN_SECTION)) / 270));
			if (mMove != null)
