...
public class SensorLightChooseView {
	...
	public SensorLightChooseView(Context context) {
		...
		ensureButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				epType = ConstUtil.DEV_TYPE_FROM_GW_LIGHT_S;
				if(lessFrameLayout.getVisibility() == View.VISIBLE && StringUtil.isNullOrEmpty(valueLessEditText.getText().toString())){
		...
		if (!StringUtil.isNullOrEmpty(value)
			...
			if (value.length() >= 2) {
				...
				if (StringUtil.equals(symbol, "<")) {
					...
					manager.setChecked(lessImageView.getId());
					moreLevels.setText(mContext.getResources().getString(R.string.house_rule_condition_device_pm_dark));
				} else {
					textViewChange(data, moreLevels);
					manager.setChecked(moreImageView.getId());
					lessLevels.setText(mContext.getResources().getString(R.string.house_rule_condition_device_pm_dark));
				}
			}
	...
	private void textViewChange(int data, TextView textView) {
		if (data < 10) {
			textView.setText(mContext.getResources().getString(R.string.house_rule_condition_device_pm_dark));
		} else if (data >= 10 && data < 50) {
			textView.setText(mContext.getResources().getString(R.string.house_rule_condition_device_pm_weak_light));
		} else if (data >= 50 && data < 150) {
			textView.setText(mContext.getResources().getString(R.string.device_link_task_sensor_degree_comfortable));
		} else if (data >= 150 && data < 750) {
			textView.setText(mContext.getResources().getString(R.string.house_rule_condition_device_pm_weak_bright));
		} else if (data > 750) {
			textView.setText(mContext.getResources().getString(R.string.house_rule_condition_device_pm_lighter));
		}
	}
}
