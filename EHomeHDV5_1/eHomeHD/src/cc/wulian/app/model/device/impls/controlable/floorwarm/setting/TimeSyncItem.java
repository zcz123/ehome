...
public class TimeSyncItem extends AbstractSettingItem {
	...
	public void showSyncResult(String returnId){
		if(!StringUtil.isNullOrEmpty(returnId)){
			if(StringUtil.equals(returnId, FloorWarmUtil.TIME_SYNC_TAG) && isTimeSynced){
				WLToast.showToast(mContext,mContext.getResources().getString(R.string.AP_setTime_equal), WLToast.TOAST_SHORT);
			}else if(StringUtil.equals(returnId, FloorWarmUtil.TIME_SYNC_FAIL_TAG) && isTimeSynced){
				WLToast.showToast(mContext,mContext.getResources().getString(R.string.common_setting_fail), WLToast.TOAST_SHORT);
			}
		}
	}
		...
		builder.setTitle(mContext.getResources().getString(R.string.operation_title))
				...
				.setListener(new WLDialog.MessageListener() {
					...
					public void onClickPositive(View view) {
						String currentTime = FloorWarmUtil.TIMESTAMP_CMD.format(new Date());
						Log.i("TimeSyncItem", currentTime);
						String timeCmd = getTimeCmd(currentTime);
						Log.i("TimeSyncItem", timeCmd);
						SendMessage.sendControlDevMsg(gwID, devID, ep, epType, FloorWarmUtil.TIME_SYNC_CMD+timeCmd);
						isTimeSynced = true;
					}
