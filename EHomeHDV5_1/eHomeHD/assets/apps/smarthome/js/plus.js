		...
		cbUtil.prototype = {
			...
			execCallback: function(key, result, type, remove) {
				...
				if (typeof(callBack[type]) == "function") {
					callBack[type](decodeURI(result));
				}
			}
		};
...
(function() {
	if (!plus.event) {
		...
		eventUtils.prototype = {
			...
			fireEventListener: function(result) {
				try {
					...
					console.log(eventUtils);
				}
			},
			removeEventListener:function (eventType, callback) {
				var listeners = this.eventListenerMap[eventType];
				if(listeners) {
		...
		window.plus.event = new eventUtils();
		window.plus.key = window.plus.event;
	}
})();
...
(function() {
	if (!window.plus.net) {
		...
		window.plus.net.XMLHttpRequest = XMLHttpRequest;
	}
})();
...
(function() {
	...
	if (!plus.webView) {
		...
		webViewEntity.prototype = {
			...
			canBack:function (queryCallback) {
				var callbackID = plus.callbackUtil.getCallbackId(function (ret) {
					var event = JSON.parse(ret);
					queryCallback(event);
				}, function (ret) {
					var event = {canBack:false};
					queryCallback(event);
				});
				var data = [callbackID, this.id];
				plus.bridge.exec("webView", "canBackWithID", JSON.stringify(data));
			}
		};
		...
		webView.prototype = {
			...
			destroyContainer:function () {
				plus.bridge.exec("webView", "destroyContainer", "");
			}
		};
		window.plus.webView = new webView();
		//兼容老的h5+的写法
		window.plus.webview = window.plus.webView;
	}
})();
...
(function() {
	if (!window.plus.nativeUI) {
		...
		window.plus.net.nativeUI = new nativeUI();
	}
})();
